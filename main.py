...
# Solve the problem
solBtn = Button(root, text="Solve", command= lambda: WPS_solver(peopleFile.get(),int(salaryFile.get()),prtVal,btVal,postVal)).grid(row=4, column=0, columnspan=6,sticky=N+S+E+W)
...
solBtn1 = Button(root, text="Solve GA", command= lambda: WPS_solver_ga(peopleFile.get(),int(salaryFile.get()),prtVal,btVal,postVal,int(populationGA.get()),int(generationGA.get()))).grid(row=7, column=0, columnspan=6,sticky=N+S+E+W)

# Week requirement file insert
weekFile = StringVar()
weekFile.set("emp_req.csv")
...
# Over-time salary
otFile = StringVar()
otVal = int(salaryFile.get())/3
otFile.set(str(int(otVal)))
otLabel = Label(root,text="Enter your overtime salary:").grid(row=9,sticky="W")
...
solBtn2 = Button(root, text="Solve Week Problem", command= lambda: WPS_solver_week(weekFile.get(),int(salaryFile.get()),mwSlider.get(),bdSlider.get(),int(otFile.get()))).grid(row=10, column=0, columnspan=6,sticky=N+S+E+W)
